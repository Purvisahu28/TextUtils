...
export default function (props) {
 ...
 <h3 >{props.heading} </h3>   
<div className="mb-3">
  <textarea className="form-control" style={{backgroundColor:props.mode==='dark'?'grey':'white',color:props.mode==='dark'?'white':'#151730'}} onChange={handleOnChange}  value={text} id="myBox" rows="10"  ></textarea>
  </div> 
  <button className="btn btn-secondary my-1"  onClick={handleUpClick}>convert to uppercase </button>
  <button className="btn btn-secondary mx-1 my-1" onClick={handlelowClick}>convert to lowercase </button>
  <button className="btn btn-secondary mx-1 my-1" onClick={handleclrtext}>clear text </button>
  <button className="btn btn-secondary mx-1 my-1" onClick={handleCopy}>Copy text</button>
  <button className="btn btn-secondary mx-1 my-1" onClick={handleSpace}>remove extra space </button>
</div>
